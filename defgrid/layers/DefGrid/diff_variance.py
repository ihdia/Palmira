...
from defgrid.layers.DefGrid.mean_feature.mean_feature import get_grid_mean_feature
from defgrid.layers.DefGrid.variance_function_atom.line_distance_func_topk.utils import (
...
class LatticeVariance(nn.Module):
    ...
    def forward(
        ...
        lattice = torch.gather(input=gather_input, dim=1, index=gather_index)  # [None, 338, 3, 2]
    ...
    def variance(self, lattice, img_fea, inference, img_pos=None, grid_size=None):
        ...
        img_fea = img_fea.reshape(n_batch, -1, img_fea.shape[-1])  # [None, 784, 5]
