...
def averaged_hausdorff_distance(set1, set2, max_ahd=np.inf):
    """
    Compute the Averaged Hausdorff Distance function between two unordered sets
    of points (the function is symmetric). Batches are not supported, so
    squeeze your inputs first!
    ...
    assert set1.ndim == 2, f'got {set1.ndim}'
    assert set2.ndim == 2, f'got {set2.ndim}'

    assert (
        set1.shape[1] == set2.shape[1]
    ...
    res_ahd = np.average(np.min(d2_matrix, axis=0)) + np.average(np.min(d2_matrix, axis=1))
    res_hd = np.max(np.min(d2_matrix, axis=0)) + np.max(np.min(d2_matrix, axis=1))
    res_hd95 = np.percentile(np.min(d2_matrix, axis=0), 95) + np.percentile(np.min(d2_matrix, axis=1), 95)

    return res_ahd, res_hd, res_hd95
...
class AveragedHausdorffLoss(nn.Module):
    ...
    def forward(self, set1, set2):
        """
        Compute the Averaged Hausdorff Distance function between two unordered
        sets of points (the function is symmetric). Batches are not supported,
        so squeeze your inputs first!
        ...
        assert set1.ndimension() == 2, f'got {set1.ndimension()}'
        assert set2.ndimension() == 2, f'got {set2.ndimension()}'
...
class WeightedHausdorffDistance(nn.Module):
    def __init__(
        self, resized_height, resized_width, return_2_terms=False, device=torch.device('cuda')
    ):
        """
        ...
        self.resized_size = torch.tensor(
            [resized_height, resized_width], dtype=torch.get_default_dtype(), device=device
        )
    ...
    def forward(self, prob_map, gt, orig_sizes):
        """
        Compute the Weighted Hausdorff Distance function between the estimated
        probability map and ground truth points. The output is the WHD averaged
        through all the batch.
