...
from defgrid.utils import dense_quad
...
class MatrixUtils:
    def __init__(self, batch_size, grid_size, grid_type, device):
        ...
        self.init_point_mask = dense_quad.create_init_point_mask(
            self.grid_size[0], self.grid_size[1]
        )
        self.triangle2point, self.init_area_mask = dense_quad.create_initial_triangle(
            self.grid_size[0], self.grid_size[1]
        )
        self.init_point_adjacent = dense_quad.create_init_point_adjacent(
            self.grid_size[0], self.grid_size[1]
        )
        self.init_cell_adjacent = dense_quad.create_init_cell_adjacent(
            self.grid_size[0], self.grid_size[1]
        )
        self.init_cell_adjacent_my = dense_quad.create_init_cell_adjacent_jun_faster(
            self.triangle2point
        )
        (
            self.init_edge_ex2,
        ...
        ) = dense_quad.create_edge_idx(self.triangle2point.data.numpy(), self.init_point.shape[0])
        ...
        self.init_triangle2point = (
            self.triangle2point.unsqueeze(0).repeat(self.batch_size, 1, 1).long().to(device)
        )
        self.init_area_mask = (
            self.init_area_mask.unsqueeze(0).repeat(self.batch_size, 1).float().to(device)
        )
        self.init_point_adjacent = (
            self.init_point_adjacent.unsqueeze(0).repeat(self.batch_size, 1, 1).float().to(device)
        )
        ...
        self.init_triangle_mask = (
            torch.zeros(self.batch_size, self.init_area_mask.shape[1]).byte().to(device)
        )
    ...
    def get_new_matrix(
        ...
        init_point = dense_quad.create_init_point(
        ...
        triangle2point, init_area_mask = dense_quad.create_initial_triangle(
        ...
        init_point_adjacent = (
            init_point_adjacent.unsqueeze(0).repeat(batch_size, 1, 1).float().to(device)
        )
        init_cell_adjacent_my = (
            init_cell_adjacent_my.unsqueeze(0).repeat(batch_size, 1, 1, 1).float().to(device)
        )
